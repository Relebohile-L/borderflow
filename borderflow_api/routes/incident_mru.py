...
incident_mru_bp = Blueprint("incident_mru", __name__)

# GET all incidents
@incident_mru_bp.route("/incidents", methods=["GET"])
def get_incidents():
    db = get_db()
...
# GET incident by ID
@incident_mru_bp.route("/incidents/<int:incident_id>", methods=["GET"])
...
@incident_mru_bp.route("/incidents", methods=["POST"])
def create_incident():
    data = request.json
    db = get_db()
    cursor = db.cursor(dictionary=True)
    ...
    cursor.close()
    db.close()
    return jsonify({"incident_id": incident_id, "site": "MRU", "status": "created"}), 201

# PUT update incident
@incident_mru_bp.route("/incidents/<int:incident_id>", methods=["PUT"])
...
# DELETE incident
@incident_mru_bp.route("/incidents/<int:incident_id>", methods=["DELETE"])
