...
milestone_sa_bp = Blueprint("milestone_sa", __name__)

@milestone_sa_bp.route("/milestones", methods=["GET"])
def get_milestones():
    db = get_db()
...
@milestone_sa_bp.route("/milestones/<int:milestone_id>", methods=["GET"])
...
@milestone_sa_bp.route("/milestones", methods=["POST"])
def create_milestone():
    ...
    cursor.close()
    db.close()
    return jsonify({"milestone_id": milestone_id, "site": "SA", "status": "created"}), 201

@milestone_sa_bp.route("/milestones/<int:milestone_id>", methods=["PUT"])
...
@milestone_sa_bp.route("/milestones/<int:milestone_id>", methods=["DELETE"])
