...
trip_mru_bp = Blueprint("trip_mru", __name__)

@trip_mru_bp.route("/trips", methods=["GET"])
def get_trips_mru():
    db = get_db(database="borderflow_mru")
...
@trip_mru_bp.route("/trip", methods=["POST"])
def create_trip_mru():
    data = request.json
    ...
    # -----------------------------
    # REPLICATION LOG (FIXED TO MATCH SA)
    # -----------------------------
    payload = {
        "trip_id": trip_id,
