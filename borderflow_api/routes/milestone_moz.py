...
milestone_moz_bp = Blueprint("milestone_moz", __name__)

@milestone_moz_bp.route("/milestones", methods=["GET"])
def get_milestones():
    db = get_db()
...
@milestone_moz_bp.route("/milestones/<int:milestone_id>", methods=["GET"])
...
@milestone_moz_bp.route("/milestones", methods=["POST"])
def create_milestone():
    ...
    cursor.close()
    db.close()
    return jsonify({"milestone_id": milestone_id, "site": "MOZ", "status": "created"}), 201

@milestone_moz_bp.route("/milestones/<int:milestone_id>", methods=["PUT"])
...
@milestone_moz_bp.route("/milestones/<int:milestone_id>", methods=["DELETE"])
